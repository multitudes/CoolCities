...
func createMockDate(hour: Int, minute: Int = 0) -> Date {
  ...
  // This simulates the "local time string parsed as UTC" issue.
  calendar.timeZone = TimeZone(secondsFromGMT: 0)!
  return calendar.date(from: DateComponents(year: 2025, month: 11, day: 2, hour: hour, minute: minute))!
}
...
#Preview("TodaysLocationView") {
  let mockWeatherData: [CurrentViewWeather] = (0...23).map { hour in
    ...
    let temp = 13.0 - (Double(hour) / 2.0) + sin(Double(hour) / 4.0) * 2.0
    return CurrentViewWeather(
      locationName: "Berlin",
      regionName: "Berlin",
      countryName: "Germany",
      date: date,
      utcOffsetSeconds: 3600, // Example offset for Berlin (CET)
